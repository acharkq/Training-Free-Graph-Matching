...
def read_mapping(path):
    with open(str(path), 'r', encoding='utf8') as f:
        lines = f.readlines()
        lines = [line.strip().split('\t') for line in lines]
        id2item = [(int(idx), item) for idx, item in lines]
...
class DBP15k(object):
    def __init__(self, directory, load_new_split, device):
        self.device = device
...
def run_dbp15k(args):
    ...
    dataset = DBP15k(directory, args.load_hard_split, device=args.device)
...
if __name__ == '__main__':
    ...
    for data in ['zh_en', 'ja_en', 'fr_en']:
        args.dataset = data
        pfms.append(run_dbp15k(args))
        torch.cuda.empty_cache()
    print('SR-TG: %.2f %.2f %.4f %.2f %.2f %.4f %.2f %.2f %.4f' % (pfms[0].top_x0[0], pfms[0].top_x0[1], pfms[0].mrr0, pfms[1].top_x0[0], pfms[1].top_x0[1], pfms[1].mrr0, pfms[2].top_x0[0], pfms[2].top_x0[1], pfms[2].mrr0))